...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				applyDefaultViewName(processedRequest, mv);//没有view则使用默认view
				mappedHandler.applyPostHandle(processedRequest, response, mv);//调用拦截器执行链中的postHandler方法
			}
			catch (Exception ex) {
	...
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
		...
		if (mv != null && !mv.wasCleared()) {
			render(mv, request, response);//视图解析
			if (errorView) {
	...
	protected void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (viewName != null) {//将viewName解析为view
			// We need to resolve the view name.
			view = resolveViewName(viewName, mv.getModelInternal(), locale, request);//视图解析器解析
			if (view == null) {
